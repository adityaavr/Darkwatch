  ...
  if (url && url.toLowerCase().includes('shein.com')) {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        await sendEvent({ type: 'PROGRESS', text: 'Initializing autonomous proxy session...' }, 1000)
        
        // 2. Stream URL
        await sendEvent({ type: 'STREAMING_URL', url: 'https://tinyfish-proxy.demo/live-view/shein-session-992' }, 500)
        
        // 3. Navigation & Actions
        await sendEvent({ type: 'ACTION', action: 'goto', text: 'https://www.shein.com' }, 1500)
        await sendEvent({ thought: 'Page loaded. Bypassing cookie consent modal.' }, 1500)
        await sendEvent({ type: 'ACTION', action: 'click', selector: '#btn-accept-cookies' }, 1000)
        
        await sendEvent({ thought: `Searching for target payload: "${query}"` }, 2000)
        await sendEvent({ type: 'ACTION', action: 'type', selector: 'input[type="search"]', text: query }, 1000)
        await sendEvent({ type: 'ACTION', action: 'click', selector: '.search-btn' }, 1000)
        
        // 4. Product Selection
        await sendEvent({ thought: 'Analyzing search results. Found optimal match.' }, 2500)
        await sendEvent({ type: 'ACTION', action: 'click', selector: '.product-list-item:first-child' }, 1500)
        
        await sendEvent({ thought: 'Evaluating product authenticity. Review cluster analysis initiated.' }, 2000)
        await sendEvent({ message: 'Trust Score calculated: 45. Signs of artificial review inflation detected.', type: 'info' }, 2000)
        
        await sendEvent({ thought: 'Adding product to cart.' }, 1500)
        await sendEvent({ type: 'ACTION', action: 'click', selector: '.add-to-cart-btn' }, 1000)
        
        // 5. Checkout & Vision Analysis
        await sendEvent({ thought: 'Proceeding to checkout phase.' }, 2000)
        await sendEvent({ type: 'ACTION', action: 'click', selector: '.checkout-btn' }, 1500)
        
        await sendEvent({ thought: 'Capturing multimodal viewport for GPT-4o Vision analysis.' }, 2500)
        await sendEvent({ type: 'PROGRESS', text: 'GPT-4o Vision: Scanning layout for deceptive UX patterns...' }, 2000)
        
        // 6. Cleansing
        await sendEvent({ message: 'WARNING: Hidden fee detected. Pre-checked "Shipping Guarantee" ($2.99).', type: 'warn' }, 3000)
        await sendEvent({ type: 'ACTION', action: 'click', selector: 'input[name="shipping_insurance"]' }, 1500)
        await sendEvent({ message: 'ACTION: Un-checked deceptive Shipping Guarantee.', type: 'success' }, 1000)
        
        await sendEvent({ message: 'WARNING: Sneaky "Priority Handling" upsell found ($1.50).', type: 'warn' }, 2000)
        await sendEvent({ type: 'ACTION', action: 'click', selector: 'button.remove-priority' }, 1500)
        await sendEvent({ message: 'ACTION: Neutralized Priority Handling fee.', type: 'success' }, 1000)
        
        // 7. Supply Chain Analysis
        await sendEvent({ thought: 'Executing Supply Chain Reality Check on product image and metadata.' }, 2000)
        await sendEvent({ message: 'Dropship alert: Item matched with AliExpress wholesale catalog ($2.50).', type: 'warn' }, 2000)
        
        await sendEvent({ thought: 'Cart sanitization complete. Finalizing payload.' }, 1500)
        
        // 8. Final Result
        const finalResult = {
          basePrice: "$15.99",
          junkFeesRemoved: [
            { name: "Shipping Guarantee", amount: "$2.99", description: "Pre-checked insurance for standard shipping." },
            { name: "Priority Handling", amount: "$1.50", description: "Hidden processing speed-up fee." }
          ],
          finalPrice: "$15.99",
          trustScore: 45,
          ...
          productOrigin: {
            ...
            analysis: "Visual search confirms identical items supplied en masse via Alibaba. The retail price represents a 540% markup over wholesale."
          }
        }
        
        await sendEvent({ type: 'COMPLETE', status: 'COMPLETED', resultJson: JSON.stringify(finalResult) }, 1000)
